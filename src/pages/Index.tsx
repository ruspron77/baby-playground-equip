...
export default function Index({ favorites, toggleFavorite, cart, addToCart, removeFromCart, updateQuantity, clearCart }: IndexProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background md:pt-[85px] py-0">
      <Header
        cart={cart}
      ...
      <section id="catalog" className={`pt-0 pb-8 bg-white order-2 md:order-2 ${catalogState.selectedCategory ? 'hidden' : ''}`}>
        <div className="container mx-auto px-3 my-2">
          <h2 className="font-heading text-center font-semibold my-0 text-3xl">Каталог продукции</h2>
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-8">
            {categories.map((category) => (
              ...
              >
                <div 
                  className="relative overflow-hidden rounded-md bg-white shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 aspect-square md:aspect-[4/3] border-2 border-gray-200 hover:border-gray-200"
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 py-1.5 px-2 -mb-px md:bottom-6 bg-[#00000000]">
                    <h3 className="font-heading text-white text-xl md:text-2xl leading-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] font-light text-center py-0 my-0">{category.name}</h3>
                  </div>
                </div>
              </div>
            ))}
